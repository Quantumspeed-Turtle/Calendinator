...
class CalendarGenerator:
    ...
    def generate_calendar(self, start_date, end_date):
        df = pd.DataFrame(columns=['CW', 'MONDAY', 'TUESDAY', 'WEDNESDAY', 'THURSDAY', 'FRIDAY'])
        ...
        while current_date <= end_date:
            ...
            df = df.append({'CW': week_number, 'MONDAY': week_days[0], 'TUESDAY': week_days[1],
                            'WEDNESDAY': week_days[2], 'THURSDAY': week_days[3], 'FRIDAY': week_days[4]}, ignore_index=True)
    ...
    def create_pdf_file(self, pdf_file_name):
        ...
        plt.savefig(pdf_file_name, bbox_inches='tight', pad_inches=0.5)
        plt.close()
